...
func TestSlidingWindowVWAPAdd(t *testing.T) {
	...
	newWithValues := func(windowCapacity int, positions []position) *SlidingWindowVWAP {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			with:          newWithValues(1, []position{{units: 10, unitPrice: 5}}),
			giveUnits:     2.5,
			giveUnitPrice: 1.2,
			expected:      1.2,
		},
		{
			...
			with:          newWithValues(2, []position{{units: 1, unitPrice: 1}, {units: 1, unitPrice: 1}}),
			giveUnits:     1,
			giveUnitPrice: 2,
		...
		},
		{
			...
			with:          newWithValues(3, []position{{units: 1, unitPrice: 1}, {units: 2, unitPrice: 1}}),
			giveUnits:     1,
			giveUnitPrice: 2,
		...
		},
		{
			...
			with:          newWithValues(3, []position{{units: 1, unitPrice: 1}}),
			giveUnits:     1,
			giveUnitPrice: 2,
			expected:      1.5, // 3 total price / 2 total units
		},
